...
The Upload page renders a [`<FileUploader>`](?path=/docs/components-input-fileuploader--default) component, as well as a separate group of [`<FilePreview>`](?path=/docs/components-filepreview--default) components for files previously uploaded. (The `<FileUploader>` component also contains `<FilePreview>` components for uploads done in the current page load)
